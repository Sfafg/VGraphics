...
int DefaultScoreFunction(
    const std::vector<Queue *> &queues, const std::set<std::string> &extensions,
    const std::set<std::string> &supportedExtensions, DeviceType type, const DeviceLimits &limits,
...
Device::Device(
    Span<Queue *const> queues, const std::set<std::string> &extensions,
    const DeviceFeatures &hintedDeviceEnabledFeatures, SurfaceHandle surface,
    std::function<
        int(PhysicalDeviceHandle id, Span<Queue *const> supportedQueues,
            const std::set<std::string> &supportedExtensions, DeviceType type, const DeviceLimits &limits,
            const DeviceFeatures &features)>
        scoreFunction
)
    ...
    for (const auto &physicalDevice : ((InstanceHandle)*vg::instance).enumeratePhysicalDevices()) {
        ...
        if (surface != nullptr && extensions.contains("VK_KHR_swapchain") &&
            supportedExtensions.contains("VK_KHR_win32_surface")) {
        ...
        for (unsigned int i = 0; i < supportedQueues.size(); i++) {
            ...
            for (unsigned int j = 0; j < queueFamilies.size(); j++) {
                ...
                if ((supportedQueues[i]->m_type & type) != supportedQueues[i]->m_type) continue;
            ...
            if (bestIndex == -1) supportedQueues[i] = nullptr;
            else queueFamilies[bestIndex].queueCount--;
        }
        ...
        if (scoreFunction != nullptr)
            score = scoreFunction(
                physicalDevice, supportedQueues, supportedExtensions, type, *(DeviceLimits *)&limits,
                *(DeviceFeatures *)&features
            );

        else
            score = DefaultScoreFunction(
                supportedQueues, extensions, supportedExtensions, type, *(DeviceLimits *)&limits,
                *(DeviceFeatures *)&features
            );
    ...
    if (highestScore == -1) std::runtime_error("No Physical Device matched the requirments.");
    ...
    for (unsigned int i = 0; i < queues.size(); i++) {
        ...
        for (unsigned int j = 0; j < queueFamilies.size(); j++) {
            ...
            // Check if family has at least all queue types required by queue.
            if ((queues[i]->m_type & type) != queues[i]->m_type) continue;
        ...
        if (queues[i]->m_index == -1) queues[i]->m_type = QueueType::None;
        else {
    ...
    vk::DeviceCreateInfo createInfo(
        vk::DeviceCreateFlags(), queueCreateInfos, nullptr, extensionsConstChar, (vk::PhysicalDeviceFeatures *)&features
    );
    ...
    for (auto &&queue : queues) {
        ...
        if (queueIndices.contains(queue->GetIndex())) queueIndices[queue->GetIndex()]++;
        else queueIndices[queue->GetIndex()] = 1;
        ...
        if (queueIndex >= maxIndex) {
            ...
            queue->m_commandPool =
                m_handle.createCommandPool({{vk::CommandPoolCreateFlagBits::eResetCommandBuffer}, queue->GetIndex()});
            queue->m_transientCommandPool = m_handle.createCommandPool(
...
Device::Device(Device &&other) noexcept : Device() { *this = std::move(other); }
...
Device::operator const DeviceHandle &() const { return m_handle; }

Device::operator const PhysicalDeviceHandle &() const { return m_physicalDevice; }

void Device::WaitUntilIdle() { vkDeviceWaitIdle(m_handle); }
...
DeviceLimits Device::GetLimits() const { return GetProperties().limits; }
...
const Queue &Device::GetQueue(uint32_t queueIndex) const { return *m_queues[queueIndex]; }
} // namespace vg
